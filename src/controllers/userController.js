...
exports.loginUser = async (req, res) => {
  try {
    ...
    // Optional: bind JWT to a tenant if provided via header
    const tenantSlug = req.header('x-tenant') || req.header('x-tenant-slug') || null;
    const token = generateJWT(user, tenantSlug || null);

    res.status(200).json({ message: 'Sesión iniciada correctamente', token, user, tenant_slug: tenantSlug });
  } catch (error) {
